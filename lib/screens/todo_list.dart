...
import 'package:flutter_application_final/screens/add_page.dart';
import 'package:http/http.dart' as http;
import '../screens2/todo_list2.dart';
          ...
          onChanged: (value) {
            Future<void> fetchItem(String id) async {
              ...
              if (response.statusCode == 201) {
                ...
                var iterar = [];
                iterar.add(jsonf2);
                final itemget = iterar[0] as Map;
                final idget = itemget['_id'] as String;
                if (value == idget) {
                ...
                ListTile(
                  ...
                  title: Text(itemget['name']),
                  subtitle: Text('Precio: \$${itemget['price']}'),
                  trailing: PopupMenuButton(
                    onSelected: (value) {
                      if (value == 'edit'){
                        navigateToAEditPage(itemget);
                ...
                child: ListTile(
                  ...
                  leading: CircleAvatar(
                    child: Image.network(
                      ...
                      '${item['img']}' // Ajustar la imagen al tamaño del contenedor
                    ),
                  ),
                  title: Text(item['name'], style: const TextStyle(fontSize: 25.0)),
                  subtitle: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Precio: \$${item['price']}', style: const TextStyle(fontSize: 15.0)),
                      Text('Categoria: ${item['categoria']}', style: const TextStyle(fontSize: 15.0)),
                    ],
                  ),
